...
(function() {
  ...
  function hideNotification() {
    if (notificationToast) {
      notificationToast.classList.remove('show', 'success', 'error', 'warning', 'info');
    }
  }
  ...
  if (submitBtn) {
    submitBtn.addEventListener('click', async function(e) {
      ...
      try {
        ...
        // 성공 메시지
        showNotification(`${name}님, 신청이 완료되었습니다!\n담당자가 확인 후 카카오톡 채팅방으로 초대해드리겠습니다.`, 'success');
        ...
        showNotification('신청 처리 중 오류가 발생했습니다.\n잠시 후 다시 시도해주세요.', 'error');
      } finally {
        // 버튼 상태 복원
